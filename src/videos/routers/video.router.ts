...
import {dbVideo} from "../../db/db";

export const videoRouter = Router({});

videoRouter
    .get("", (req, res) => {
        res.status(HttpStatus.Ok).send(dbVideo.videos);
    })
    ...
    .get("/:id", (req, res) => {
        ...
        if (isNaN(id) || !Number.isInteger(id) || id < 1) {
            return res
                ...
                .send("If video for passed id doesn`t exist");
        }
        // Поиск видео по ID
        const video = dbVideo.videos.find((v) => v.id === id);
        if (!video) {
            return res
                .status(HttpStatus.NotFound)
                .send("If video for passed id doesn`t exist");
    ...
    .post("", (req, res) => {
        ...
        // --- Успех ---
        const newVideo = {
            id: dbVideo.videos.length ? dbVideo.videos[dbVideo.videos.length - 1].id + 1 : 1,
            title: title.trim(),
            author: author.trim(),
    ...
    .put("/:id", (req, res) => {
        ...
        const videoIndex = dbVideo.videos.findIndex((v) => v.id === videoId);
        if (videoIndex === -1) {
            return res.sendStatus(HttpStatus.NotFound);
        }
        ...
        dbVideo.videos[videoIndex] = updatedVideo;
    ...
    .delete("/:id", (req, res) => {
        ...
        const videoIndex = dbVideo.videos.findIndex((v) => v.id === id);
        if (videoIndex === -1) {
            return res.status(HttpStatus.NotFound).send("Not Found");
        }

        // Удаление видео из массива
        dbVideo.videos.splice(videoIndex, 1);
